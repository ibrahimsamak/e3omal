...
class SASignUpHandy: UIViewController , CategoryProtocol,
    UICollectionViewDelegate , UICollectionViewDelegateFlowLayout ,  UICollectionViewDataSource ,UIActionSheetDelegate ,UIImagePickerControllerDelegate , UINavigationControllerDelegate
{
    ...
    @IBAction func btn_change24(_ sender: UISwitch)
    {
        if sender.isOn{
            self.is24 = 1
        }
    ...
    @IBAction func btnSignUp(_ sender: UIButton)
    {
        if MyTools.tools.connectedToNetwork()
        {
            ...
            else
            {
                ...
                let type = MyApi.userType.handyman.rawValue
                self.showIndicator()
                MyApi.api.PostSignUpNewUser(name: txtName.text!, email: txtEmail.text!, mobile:txtPhone.text! , password: txtPassword.text!, type: type, is_24: String(self.is24), company_name: txtName.text!, category_id: self.selecetedarr, profile_image: image!, civil_id: self.txtCivilID.text!, images:  self.imageArray, token: deviceToken!)
                { (response, error) in
                    if(response.result.value != nil)
                    {
                        if let JSON = response.result.value as? NSDictionary
                        {
                            ...
                            {
                                self.showOkAlert(title: "Error".localized.localized, message: JSON["message"] as? String ?? "")
                            }
                            self.hideIndicator()
                        }
                    }
